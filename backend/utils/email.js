...
async function sendOTPEmail(to, otp, type = 'verify') {
  ...
  await axios.post('https://api.brevo.com/v3/smtp/email', {
    ...
    to: [{ email: to }],
    subject,
    htmlContent: `<div style="font-family:Arial;max-width:520px;margin:0 auto;border:1px solid #e0e0e0;border-radius:10px;overflow:hidden;"><div style="background:#006400;padding:24px 32px;"><h1 style="color:#fff;margin:0;">IReV Electoral Portal</h1></div><div style="padding:32px;"><p>Use the code below to ${action}. Expires in <strong>10 minutes</strong>.</p><div style="background:#f7f8f6;border:2px solid #006400;border-radius:8px;padding:20px;text-align:center;margin:24px 0;"><span style="font-size:2.2rem;font-weight:700;letter-spacing:0.3em;color:#006400;">${otp}</span></div><p style="color:#9e9e9e;font-size:0.8rem;">If you did not request this, ignore this email.</p></div></div>`
  }, {
    headers: {
...
async function sendVotingOpenEmail(to, fullName, election) {
  await axios.post('https://api.brevo.com/v3/smtp/email', {
    sender: { name: 'IReV Portal', email: 'irevportal@gmail.com' },
    to: [{ email: to }],
    subject: `Voting is Now Open: ${election.title}`,
